...
**测试/复现过程**:
1. 直接访问目标URL: https://i1.huanqiu-ltd.com/staging
2. 无需登录认证
3. 确认可访问敏感功能或数据

**漏洞URL**: https://i1.huanqiu-ltd.com/staging
...
```html
# 使用 curl 测试常见敏感路径
curl https://i1.huanqiu-ltd.com/staging/admin
curl https://i1.huanqiu-ltd.com/staging/admin/login
curl https://i1.huanqiu-ltd.com/staging/dashboard
curl https://i1.huanqiu-ltd.com/staging/console
curl https://i1.huanqiu-ltd.com/staging/debug
curl https://i1.huanqiu-ltd.com/staging/actuator
curl https://i1.huanqiu-ltd.com/staging/actuator/env
curl https://i1.huanqiu-ltd.com/staging/swagger-ui.html
curl https://i1.huanqiu-ltd.com/staging/api-docs
...
for path in paths:
    url = f"https://i1.huanqiu-ltd.com/staging/staging"
    try:
        resp = requests.get(url, timeout=5, allow_redirects=False)
        if resp.status_code == 200:
            print(f"[发现] {url} - {resp.status_code}")
...
# 使用 ffuf 扫描
ffuf -u https://i1.huanqiu-ltd.com/staging/FUZZ -w wordlist.txt
```
