# HTTP Request Smuggling → Cache Poisoning → Stored XSS on playtika.com

## Summary

A critical HTTP Request Smuggling vulnerability was discovered on playtika.com that can be exploited to perform cache poisoning attacks, injecting stored XSS that affects ALL users visiting the affected page. The attack requires no authentication and can be executed remotely.
...
**Impact**: All users visiting the affected page
...
- **Screenshots and video evidence are attached**
