...
<body>
    ...
    <iframe src="https://www.sto.cn"></iframe>
</body>
</html>
```
...
resp = requests.get("https://www.sto.cn")
...
**证据**:
```
缺失: X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, Content-Security-Policy, Referrer-Policy
```

**PoC / Exploit**:
```html
# 使用 curl 检查响应头
curl -I https://www.sto.cn -v

# 使用 Python 检查
import requests
resp = requests.head("https://www.sto.cn")
...
### 5. 🟢 [低危] Nuclei-weak-cipher-suites

#### 漏洞标题
Sto存在weak-cipher-suites
...
```html
# 使用 openssl 检测
openssl s_client -connect www.sto.cn:443 -cipher 'LOW:EXP'

# 使用 nmap 检测
nmap --script ssl-enum-ciphers -p 443 www.sto.cn

# 使用 testssl.sh 检测
./testssl.sh www.sto.cn
